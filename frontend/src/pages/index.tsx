...
function HeroSection() {
  return (
    <section
      className="min-h-screen flex flex-col justify-center pt-24 pb-16 overflow-hidden relative"
        ...
        <div className="grid lg:grid-cols-2 gap-12 items-center">
          ...
          <div>
            ...
            <div className="flex flex-col sm:flex-row items-start gap-3 mb-4">
              <Link href="/auth/signup" className="btn-teal text-base w-full sm:w-auto justify-center">
                Start 14-day trial
              </Link>
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6">
        <div className="text-center mb-14">
          <div className="chip mb-5 mx-auto">
            <span className="mr-2">⚡</span>EVERYTHING, STITCHED TOGETHER
          </div>
          <h2
            className="font-display text-forest mb-4"
            style={{ fontFamily: 'Fraunces, serif', letterSpacing: '-0.02em', fontSize: 'clamp(32px, 5vw, 52px)', lineHeight: 1.05 }}
          >
            One dashboard.<br />
          ...
          {FEATURES.map((feature, i) => (
            ...
            >
              <div className="flex items-center gap-2 mb-4">
                ...
                <span className="text-xs font-bold tracking-widest" style={{ color: feature.color }}>{feature.tag}</span>
              </div>
              <h3 className="font-display text-forest text-xl mb-2" style={{ fontFamily: 'Fraunces, serif', letterSpacing: '-0.01em' }}>
                {feature.title}
              </h3>
              <p className="text-forest/55 text-sm leading-relaxed">{feature.description}</p>
            </div>
          ))}
        </div>
      </div>
  ...
  return (
    <section id="faq" className="py-20 bg-white">
      <div className="max-w-3xl mx-auto px-4 sm:px-6">
        <div className="text-center mb-14">
          <div className="chip mb-5 mx-auto"><span className="mr-2">?</span>QUESTIONS</div>
          <h2
            className="font-display text-forest mb-4"
        ...
        <div className="space-y-0">
          {FAQS.map((faq, i) => (
            <div key={i} className="border-b border-forest/10">
              ...
              >
                <span className="font-semibold text-forest text-base pr-4 group-hover:text-teal transition-colors duration-200">{faq.question}</span>
                <span
              ...
              {openIndex === i && (
                <div className="pb-5 text-forest/60 text-sm leading-relaxed">{faq.answer}</div>
              )}
            </div>
          ))}
        </div>
    ...
    <section className="py-24 bg-forest">
      <div className="max-w-3xl mx-auto px-4 sm:px-6 text-center">
        <h2
          ...
          style={{ fontFamily: 'Fraunces, serif', letterSpacing: '-0.02em', fontSize: 'clamp(36px, 6vw, 64px)', lineHeight: 1.0 }}
        >
          Ready to retire<br /><span className="italic">the spreadsheet?</span>
        </h2>
        <p className="text-white/50 text-lg mb-10">14 days free. No credit card. Cancel in one click.</p>
        <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
          <Link href="/auth/signup" className="btn-teal text-base w-full sm:w-auto justify-center">Start free trial</Link>
          <Link href="/pricing" className="text-white/60 hover:text-white text-sm font-semibold transition-colors duration-200 underline underline-offset-4">
            See pricing
          </Link>
        </div>
      </div>
...
export default function HomePage() {
  return (
    <>
      <Head>
        <title>Collabify — The CRM for Creators Who Hustle</title>
        <meta name="description" content="Manage brand deals, build media kits, send invoices & get AI-powered rate suggestions." />
      </Head>
